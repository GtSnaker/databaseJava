...
public class VerTablas {
		...
		try {
			...
			if (opcion < 1 || opcion > 14) {
				...
				case 12:
					...
					break;
				case 13:
					VerTablas.verJugadorXTorneo(connection);
					break;
				case 14:
					VerTablas.verJugadorXLogro(connection);
					break;
				}
			}
	...
	public static void verJugadorXLogro(DbConnection connection)
			throws ClassNotFoundException, SQLException {
		JugadorXLogroDAO miJugadorXLogroDAO = new JugadorXLogroDAO();
		JugadorXLogroVO miJugadorXLogro;
		ArrayList<JugadorXLogroVO> jugadores = miJugadorXLogroDAO
				.ver(connection);
		if (jugadores.size() > 0) {
			for (int i = 0; i < jugadores.size(); i++) {
				miJugadorXLogro = jugadores.get(i);
				System.out.println("IdJugador: " + miJugadorXLogro.getIdJugador()
						+ ", IdLogro: " + miJugadorXLogro.getIdLogro()
						);
			}
			System.out.println("");
		} else {
